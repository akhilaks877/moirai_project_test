							...
							<div class="main-card mb-3 card">
								<div class="card-body">
								   ...
								   <button class="btn btn-info run-tests" style="display: none;">Start</button>
									...
									<form id="exercise_form" action="{{route('student.title.check_answers')}}" method="post" class="dropzone db-clickable" enctype="multipart/form-data">
										@csrf
										<input type="hidden" name="exer_ent" value="{{ $exer_data->id }}" readonly>
										<input type="hidden" name="question_type" value={{$question_type}} id="question_type">
										<input type="hidden" name="chapter_id" value={{$chap}} id="chapter_id">
										...
										<div class="row">
											<div class="col-lg-12 text-right">
												<hr>
												@if($type!=='Examination Exercise')
												<button class="mt-1 btn btn-primary" name="commit" type="submit">Save the exercise</button>
												<input type="hidden" name="exam-status" value="{{$exam_attended}}">
												@endif

												@if($type == 'Examination Exercise')
												<button class="mt-1 btn btn-primary exerz" name="commit" type="submit">Save and publish</button>
												<input type="hidden" name="exam-status" value="0">
												@endif
...
$(function(){
 ...
 function startsTest()
 {
	 ...
	 console.log("in starts test");
	 $excerid=$('input[type="hidden"][name="exer_ent"]').val(); 
	 var now_dat= new Date(); $start_time=now_dat.getFullYear() +"-"+ (now_dat.getMonth() + 1) +"-"+ now_dat.getDate() + " " + now_dat.getHours() + ":" + now_dat.getMinutes() + ":" + now_dat.getSeconds();
	 console.log( $excerid);
...
//  $('.exerz').on('click',function(e)
//  {
// 	e.preventDefault();
// 	var data = $('#exercise_form').serializeArray();
// 	console.log(data);
// 	$.ajax({
//   url:'{{ route('student.title.check_answers') }}',
//   headers: {
//       'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
//   },
//  type: "POST",
