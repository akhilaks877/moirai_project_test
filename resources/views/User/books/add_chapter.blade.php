
<div class="alert dimension alert-danger" style="display: none;"><strong></strong>.</div>
@if(session()->has('chapstatus.status'))
    <div class="alert alert-{{ session('chapstatus.status') }}">
    {!! session('chapstatus.message') !!}
    </div>
@endif
<a data-toggle="collapse" href="#reading_nav" class="btn-open-options btn btn-primary button_expand_mobile" aria-expanded="false"><span class="fa fa-indent fa-w-16"></span></a>
    <div class="parent_header_reading">
        <div class="header_reading" style="background-color: #dcdcdc!important; z-index: 10; width: 100%;">
            <div class="row navbar_uncollapsed_desktop">
                <div class="col-md-9">
                    <h1 class="ml-2 mt-2 title_chapter_nav">{{ ucfirst($data['book']->title) }} > {{ __('messages.add_chapter')  }}</h1>
                </div>
                <div class="col-md-3">
                    <a data-toggle="collapse" href="#reading_nav" class="btn btn-primary float-right m-2" aria-expanded="true">{{__('menus.book_navigation')}}</a>
                </div>
            </div>

            <div class="collapse row" id="reading_nav" style="">
                <div class="col-md-12">
                    <hr class="m-0">
                </div>
                <div class="col-md-8 pr-0 menu_separator">

                    <div class="card-body pr-0">

                        <h2 class="card-title">{{__('messages.table_of_contents')}}</h2>
                        <div class="scroll-area-sm">
                            <div class="scrollbar-container ps--active-y ps">
                                <ul class="book_index p-0">
                                    @foreach ($data['chapters'] as $k=>$chap)
                                     <li><a href="{{ route('admin.title.manage_book_note',['id'=>$data['book']->id,'type'=>'edit','chapter'=>$chap->id]) }}">Chapter {{$k+1}}: {{ $chap->title}}</a></li>
                                    @endforeach
                                </ul>
                            </div>
                        </div>
                    </div>
                </div>
                <div class="col-md-4">
                    <div class="row">
                        <div class="position-relative col-12">
                            <div class="col-10">
                            <p class="d-inline-block  pl-2">{{__('messages.font_size')}} </p>
                            <a href="javascript:void(0);" class="manip bigger_font">A<sup>+</sup></a>
                            <a href="javascript:void(0);" class="manip lower_font">A<sup>-</sup></a>
                            <input type="hidden" name="selected_font_size" value="{{ $pref_setts->selected_font_size }}">
                            </div>
                        </div>
                    </div>
                    <div class="position-relative mb-1 row">
                        <div class="col-9">
                            <label class="form-check-label pl-2" for="note_editeur">{{__('messages.view_editors_note')}}</label>
                        </div>
                        <div class="col-2">
                            <input type="checkbox" id="note_editeur" class="note_sett" name="view_editor_note" value="{{ ($pref_setts->view_editor_note == 1)?1:2 }}" @if($pref_setts->view_editor_note == 1) checked @endif checked data-toggle="toggle" data-size="small" data-onstyle="success" data-offstyle="primary">
                        </div>
                    </div>
                    <div class="position-relative mb-1 row">
                        <div class="col-9">
                            <label class="form-check-label  pl-2" for="note_teacher">{{__('messages.view_teachers_note')}}</label>
                        </div>
                        <div class="col-2">
                            <input type="checkbox" id="note_teacher" class="note_sett" name="view_teacher_note" value="{{ ($pref_setts->view_teacher_note == 1)?1:2 }}" @if($pref_setts->view_teacher_note == 1) checked @endif data-toggle="toggle" data-size="small" data-onstyle="success" data-offstyle="primary">
                        </div>
                    </div>
                    <div class="position-relative mb-1 row">
                        <div class="col-9">
                            <label class="form-check-label  pl-2" for="note_perso">{{__('messages.view_my_note')}}</label>
                        </div>
                        <div class="col-2">
                            <input type="checkbox" id="note_perso" class="note_sett" name="view_student_note" value="{{ ($pref_setts->view_student_note == 1)?1:2 }}" @if($pref_setts->view_student_note == 1) checked @endif data-toggle="toggle" data-size="small" data-onstyle="success" data-offstyle="primary">
                        </div>
                    </div>
                    <div class="position-relative mb-3 row">
                        <div class="col-9">
                            <label class="form-check-label  pl-2" for="note_add">{{__('messages.enable_note_editing')}}</label>
                        </div>
                        <div class="col-2">
                            <input type="checkbox" id="note_add" class="note_sett" name="enable_note_edit" value="{{ ($pref_setts->enable_note_edit == 1)?1:2 }}" @if($pref_setts->enable_note_edit == 1) checked @endif data-toggle="toggle" data-size="small" data-onstyle="success" data-offstyle="primary">
                        </div>
                    </div>
                </div>
            </div>

            <div class="col-md-12" id="reading_percentage">
                <div class="bar" aria-valuenow="25" aria-valuemin="0" aria-valuemax="100" style="width: 25%;" role="progressbar"></div>
            </div>

        </div>
    </div>
    <div class="tab-content size_temoin">
        <div class="tab-pane tabs-animation fade show active">
            <div class="main-card mb-3 card">
                <div class="card-body">


                    <h2 class="title_chapter">Chapter 2: Lorem Ipsum dolor sit amet</h2>
                    <form class="" action="" method="POST" data-url="{{ route('admin.title.add_upnotes',['id'=> $data['book']->id]) }}" id="addnewNote" enctype="multipart/form-data"  data-parsley-validate>
                     <input type="hidden" name="book_entity" value="{{ $data['book']->id }}" readonly/>
                      <div class="position-relative form-group">
                      <h5 class="req">{{__('messages.chapter_name')}}</h5>
                      <input type="text" name="chapter_name" class="form-control" data-parsley-required data-parsley-error-message="Chapter name is required">
                      </div>
                      <div id="notes_container">
                      <div class="main-card note-card mb-3 card"><div class="card-body"><h4 class="covrd">Note 1</h4>
                        <input type="hidden" name="note_element[]" value="1" readonly/>
                      <div class="position-relative form-group">
                        <h5>{{__('messages.title')}}</h5>
                        <input type="text" name="title[]" class="form-control">
                      </div>
                      <div class="position-relative form-group">
                        <h5>{{__('messages.subtitle')}}</h5>
                        <input type="text" name="subtitle[]" class="form-control">
                      </div>
                      <div class="position-relative form-group">
                        <h5>{{__('messages.add_a_note')}}</h5>
                        <textarea class="textarea_block" name="note[]"></textarea>
                      </div>
                      <div class="position-relative form-group">
                        <h5>{{__('messages.add_a_file')}}</h5>
                        <input type="hidden" name="contain_file[]" value="0" readonly> <!--- check input file contains file or not --->
                        <span class="spns pe-7s-close-circle"></span>
                        <input name="notefile[]" class="exampleFile" accept="image/*" type="file" class="form-control-file mt-2">
                        <div class="imgPreview"></div>
                        <input type="text" class="imgCaption" name="filecaption[]" style="display:none;" placeholder="Image Caption"/>
                      </div>
                      </div></div></div>
                      <div class="row">
                      <div class="col-sm-3"></div>
                      <div class="col-sm-3"></div>
                      <div class="col-sm-3">
                        <a href="javascript:void(0);" class="mt-2 btn btn-primary float-right add_another_note">Add Another Note</a>
                      </div>
                      <div class="col-sm-3">
                        <button type="submit" class="mt-2 btn btn-primary float-right">{{__('menus.complete_chapter')}}</button>
                      </div>
                      </div>
                    </form>
                    {{-- <h4>This is the subtitle of the paragraph</h4>
                    <p>
                    Lorem ipsum dolor sit amet, Lorem consectetur adipiscing elit Lorem. Aliquam venenatis neque ac enim ultrices, ac bibendum erat facilisis. Ut vel nisi at lacus elementum facilisis. Praesent a libero tincidunt, pharetra ipsum a, convallis ligula. Morbi iaculis turpis sed arcu efficitur condimentum. Curabitur dignissim fringilla nisl quis pellentesque. Nullam vehicula lectus sit amet vehicula convallis. In ultrices sodales enim vitae maximus. Pellentesque lectus tortor, lacinia at rutrum at, pretium sit amet dui. Duis enim metus, ultrices vel purus eget, auctor aliquet felis. Curabitur purus dui, rhoncus quis lobortis ac, porttitor eu eros. Integer ac erat ligula. Ut diam tortor, feugiat sit amet enim sed, iaculis tempus lectus. Orci varius natoque penatibus et magnis dis parturient montes, nascetur ridiculus mus. Donec sodales cursus lectus, ornare fermentum enim.
                    <a data-toggle="collapse" href="#collapse_note1" class="btn-open-options btn btn-primary float-right add_note" aria-expanded="false"><span class="fa fa-plus fa-w-16"></span> {{__('menus.add_a_note')}}</a>
                    </p>
                    <form>
                        <div class="collapse row" id="collapse_note1">
                                <div class="col-12">
                                    <label for="add_note1" class="">{{__('messages.add_a_note')}}</label>
                                    <textarea name="text" id="add_note1" class="form-control"></textarea>
                                </div>
                                <div class="col-4">
                                </div>
                                <div class="col-2">
                                    <label for="exampleFile1" class="float-right mt-2 label_file">{{__('messages.add_a_file')}}</label>
                                </div>
                                <div class="col-3">
                                    <input name="file" id="exampleFile1" type="file" class="form-control-file mt-2">
                                </div>
                                <div class="col-3">
                                    <button class="mt-2 btn btn-primary float-right">{{__('menus.save')}}</button>
                                </div>
                        </div>
                    </form> --}}
                    {{-- <p>Aenean id commodo felis. Nunc luctus mauris nibh, a tincidunt lacus volutpat eget. Integer malesuada lorem lorem, pellentesque imperdiet tellus condimentum quis. Vivamus vitae massa libero. Fusce blandit diam nec metus ornare, sit amet efficitur velit suscipit. Vivamus sed dui maximus, ornare velit id, scelerisque nisl. Praesent id ligula enim. Fusce at sollicitudin neque. Nulla non vulputate neque. Phasellus mauris enim, ultricies luctus odio vel, tempus rhoncus turpis. Proin euismod sem sit amet urna venenatis, vitae finibus ex auctor. Nullam eu ligula nisl. Orci varius natoque penatibus et magnis dis parturient montes, nascetur ridiculus mus. Sed mollis risus sit amet bibendum maximus.
                    <a data-toggle="collapse" href="#collapse_note2" class="btn-open-options btn btn-primary float-right add_note" aria-expanded="false"><span class="fa fa-plus fa-w-16"></span> {{__('menus.add_a_note')}}</a>
                    </p> --}}
                    {{-- <form>
                        <div class="collapse row" id="collapse_note2">
                                <div class="col-12">
                                    <label for="add_note2" class="">{{__('messages.add_a_note')}}</label>
                                    <textarea name="text" id="add_note2" class="form-control"></textarea>
                                </div>
                                <div class="col-4">
                                </div>
                                <div class="col-2">
                                    <label for="exampleFile2" class="float-right mt-2 label_file">{{__('messages.add_a_file')}}</label>
                                </div>
                                <div class="col-3">
                                    <input name="file" id="exampleFile2" type="file" class="form-control-file mt-2">
                                </div>
                                <div class="col-3">
                                    <button class="mt-2 btn btn-primary float-right">{{__('menus.save')}}</button>
                                </div>
                        </div>
                    </form> --}}
                    {{-- <div class="row">
                        <div class="col-md-12 d-flex">
                            <a href="#" class="mt-2 ml-1 btn btn-primary mx-auto">{{__('menus.complete_chapter')}}</a>
                        </div>
                    </div> --}}
                </div>
            </div>
        </div>
    </div>

